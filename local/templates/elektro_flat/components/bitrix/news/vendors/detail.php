...
if(0 < intval($arResult["SECTION_INFO"]["ELEMENT_ID"])) {
	$arFilter["ID"] = $arResult["SECTION_INFO"]["ELEMENT_ID"];
} elseif("" != $arResult["SECTION_INFO"]["ELEMENT_CODE"]) {
	$arFilter["CODE"] = $arResult["SECTION_INFO"]["ELEMENT_CODE"];
}
...
if($obCache->InitCache($arParams["CACHE_TIME"], $cache_id, $cache_dir)) {
	...
	$rsElement = CIBlockSection::GetList(Array($by=>$order), $arFilter, true);
